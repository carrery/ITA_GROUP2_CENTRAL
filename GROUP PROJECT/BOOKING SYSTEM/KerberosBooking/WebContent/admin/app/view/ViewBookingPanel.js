/*
 * File: app/view/ViewBookingPanel.js
 *
 * This file was generated by Sencha Architect version 4.2.2.
 * http://www.sencha.com/products/architect/
 *
 * This file requires use of the Ext JS 5.1.x library, under independent license.
 * License of Sencha Architect does not include license for Ext JS 5.1.x. For more
 * details see http://www.sencha.com/license or contact license@sencha.com.
 *
 * This file will be auto-generated each and everytime you save your project.
 *
 * Do NOT hand edit this file.
 */

Ext.define('KerberosBooking.view.ViewBookingPanel', {
    extend: 'Ext.panel.Panel',
    alias: 'widget.ViewBookingPanel',
    fieldDefaults: {
        readOnly : true //all fiels are in readOnly mode
    },
    alternateClassName: [
        'ViewBookingPanel'
    ],
    requires: [
        'Ext.Img',
        'Ext.form.Panel',
        'Ext.form.field.ComboBox',
        'Ext.form.Label',
        'Ext.form.field.Date',
        'Ext.grid.Panel',
        'Ext.toolbar.Toolbar',
        'Ext.button.Button',
        'Ext.grid.column.Column',
        'Ext.view.Table',
        'Ext.grid.plugin.CellEditing',
        'Ext.selection.CheckboxModel',
        'Ext.form.CheckboxGroup',
        'Ext.form.field.Checkbox',
        'Ext.toolbar.Fill'
    ],
    viewModel: {
        data: {
            shipment: {},
    		container: {}
        }
    },
    initComponent: function() {
        this.callParent(arguments);
        var me = this;
        KerberosBooking.model.AllBookingsModel.load(1, {
            success: function(record, operation) {
                me.getViewModel().set('shipment', record);
            }
        })
    },
    id: 'ViewBookingPanel',
    itemId: 'ViewBookingPanel',
    scrollable: true,
    defaultListenerScope: true,

    items: [
        {
            xtype: 'container',
            flex: 1,
            height: 900,
            layout: {
                type: 'hbox',
                align: 'stretch'
            },
            items: [
                {
                    xtype: 'image',
                    dock: 'top',
                    flex: 1,
                    height: 201,
                    hidden: true,
                    width: 201,
                    src: 'resources/images/englishmap.png'
                }
            ],
            listeners: {
                afterrender: 'onContainerAfterRender'
            }
        },
        {
            xtype: 'container',
            scrollable: true,
            layout: {
                type: 'vbox',
                align: 'stretch'
            },
            items: [
                {
                    xtype: 'form',
                    id: 'createBookingPanel',
                    itemId: 'createBookingPanel',
                    layout: 'auto',
                    bodyPadding: 10,
                    title: 'Create Booking',
                    titleAlign: 'left',
                    items: [
                        {
                            xtype: 'container',
                            flex: 1,
                            id: 'bookingPanel',
                            itemId: 'bookingPanel',
                            padding: 10,
                            layout: {
                                type: 'hbox',
                                align: 'stretch',
                                padding: 10
                            },
                            items: [
                                {
                                    xtype: 'textfield',
                                    flex: 1,
                                    id: 'bookingNumber',
                                    itemId: 'bookingNumber',
                                    padding: 10,
                                    fieldLabel: 'Booking Number',
                                    bind: '{shipment.shipmentNum}',
                                    name: 'bookingNumber'
                                },
                                {
                                    xtype: 'combobox',
                                    flex: 1,
                                    id: 'bookingOffice',
                                    itemId: 'bookingOffice',
                                    padding: 10,
                                    fieldLabel: 'Booking Office',
                                    name: 'bookingOffice',
                                    store: [
                                        'MNL',
                                        'HKG',
                                        'ZHA',
                                        'USA'
                                    ]
                                },
                                {
                                    xtype: 'label',
                                    flex: 1,
                                    padding: 10,
                                    text: 'Booking Status'
                                },
                                {
                                    xtype: 'textfield',
                                    flex: 1,
                                    id: 'bookingStatus',
                                    itemId: 'bookingStatus',
                                    padding: 10,
                                    fieldLabel: 'Booking Status',
                                    bind: '{shipment.shipmentStatus}',
                                    name: 'bookingNumber'
                                },
                                
                            ]
                        },
                        {
                            xtype: 'container',
                            flex: 1,
                            id: 'bookingInfoPanel',
                            itemId: 'bookingInfoPanel',
                            padding: 10,
                            layout: {
                                type: 'hbox',
                                align: 'middle',
                                padding: 10
                            },
                            items: [
                                {
                                    xtype: 'container',
                                    items: [
                                        {
                                            xtype: 'textfield',
                                            id: 'bookingShipper',
                                            itemId: 'bookingShipper',
                                            padding: 10,
                                            fieldLabel: 'Shipper',
                                            bind: '{shipment.shipper}',
                                            labelWidth: 120,
                                            name: 'shipper'
                                        },
                                        {
                                            xtype: 'datefield',
                                            padding: 10,
                                            fieldLabel: 'From Date',
                                            bind: '{shipment.fromDate}',
                                            labelWidth: 120,
                                            format: 'Y-m-d',
                                            name: 'fromDate'
                                        }
                                    ]
                                },
                                {
                                    xtype: 'container',
                                    items: [
                                        {
                                            xtype: 'textfield',
                                            id: 'bookingConsignee',
                                            itemId: 'bookingConsignee',
                                            padding: 10,
                                            fieldLabel: 'Consignee',
                                            bind: '{shipment.consignee}',
                                            labelWidth: 120,
                                            name: 'consignee'
                                        },
                                        {
                                            xtype: 'datefield',
                                            padding: 10,
                                            fieldLabel: 'To Date',
                                            bind: '{shipment.toDate}',
                                            labelWidth: 120,
                                            format: 'Y-m-d',
                                            name: 'toDate'
                                        }
                                    ]
                                }
                            ]
                        },
                        {
                            xtype: 'container',
                            id: 'containerPanel',
                            itemId: 'containerPanel',
                            padding: 10,
                            layout: {
                                type: 'vbox',
                                align: 'stretch',
                                padding: 10
                            },
                            items: [
                                {
                                    xtype: 'container',
                                    flex: 1,
                                    layout: {
                                        type: 'hbox',
                                        align: 'stretch'
                                    },
                                    items: [
                                        {
                                            xtype: 'container',
                                            items: [
                                                {
                                                    xtype: 'combobox',
                                                    id: 'bookingFromCity',
                                                    itemId: 'bookingFromCity',
                                                    padding: 10,
                                                    bind: '{shipment.fromCity}',
                                                    fieldLabel: 'From City',
                                                    labelWidth: 120,
                                                    name: 'fromCity',
                                                    store: [
                                                        'MNL',
                                                        'HKG',
                                                        'ZHA',
                                                        'USA',
                                                        'LGB',
                                                        'SIN',
                                                        'PUS',
                                                        'RTM'
                                                    ],
                                                    listeners: {
                                                        blur: 'onBookingFromCityBlur'
                                                    }
                                                },
                                                {
                                                    xtype: 'combobox',
                                                    id: 'bookingCargoNature',
                                                    itemId: 'bookingCargoNature',
                                                    padding: 10,
                                                    fieldLabel: 'Cargo Nature',
                                                    labelWidth: 120,
                                                    name: 'cargoNature',
                                                    store: [
                                                        'GC',
                                                        'RF',
                                                        'AW',
                                                        'DG'
                                                    ]
                                                },
                                                {
                                                    xtype: 'textfield',
                                                    id: 'bookingCargoDesc',
                                                    itemId: 'bookingCargoDesc',
                                                    padding: 10,
                                                    fieldLabel: 'Cargo Description',
                                                    labelWidth: 120,
                                                    name: 'cargoDesc'
                                                }
                                            ]
                                        },
                                        {
                                            xtype: 'container',
                                            padding: '',
                                            items: [
                                                {
                                                    xtype: 'combobox',
                                                    id: 'bookingToCity',
                                                    itemId: 'bookingToCity',
                                                    padding: 10,
                                                    fieldLabel: 'To City',
                                                    bind: '{shipment.toCity}',
                                                    labelWidth: 120,
                                                    name:'toCity',
                                                    store: [
                                                        'MNL',
                                                        'HKG',
                                                        'ZHA',
                                                        'USA',
                                                        'LGB',
                                                        'SIN',
                                                        'PUS',
                                                        'RTM'
                                                    ],
                                                    listeners: {
                                                        blur: 'onBookingToCityBlur'
                                                    }
                                                },
                                                {
                                                    xtype: 'combobox',
                                                    id: 'bookingConHS',
                                                    itemId: 'bookingConHS',
                                                    padding: 10,
                                                    fieldLabel: 'HS Code',
                                                    labelWidth: 120,
                                                    name: 'containerHS',
                                                    store: [
                                                        '10 - Cereal Grains',
                                                        '50 - Silk & Silk Fabrics'
                                                    ]
                                                }
                                            ]
                                        }
                                    ]
                                },
                                {
                                    xtype: 'container',
                                    flex: 1,
                                    items: [
                                        {
                                            xtype: 'gridpanel',
                                            title: '',
                                            height: 177,
                                            id: 'createContainer',
                                            store: 'createBooking',
                                            columns: [
                                                {
                                                	xtype: 'gridcolumn',
                                                    flex: 1,
                                                    id: 'bookingConRef',
                                                    itemId: 'bookingConRef',
                                                    dataIndex: 'refNum',
                                                    text: 'Reference Number'
                                                },
                                                {
                                                	xtype: 'gridcolumn',
                                                    dataIndex: 'cntrType',
                                                    flex: 1,
                                                    id: 'bookingConNumber',
                                                    itemId: 'bookingConNumber',
                                                    text: 'Container Type',
                                                    editor: {
                                                        xtype: 'combobox',
                                                        name: 'cntrType',
                                                        store: [
                                                            '20GP',
                                                            '40GP',
                                                            '20HQ',
                                                            '40HQ',
                                                            '45HQ'
                                                        ]
                                                    }
                                                },
                                                {
                                                	xtype: 'gridcolumn',
                                                    flex: 1,
                                                    dataIndex: 'grossWt',
                                                    text: 'Gross Weight',
                                                    editor: {
                                                        xtype: 'textfield',
                                                        name: 'grossWt'
                                                    }
                                                },
                                                {
                                                	xtype: 'gridcolumn',
                                                    flex: 1,
                                                    dataIndex: 'netWt',
                                                    text: 'Net Weight',
                                                    editor: {
                                                        xtype: 'textfield',
                                                        name: 'netWt'
                                                    }
                                                },
                                                {
                                                	 xtype: 'gridcolumn',
                                                     flex: 1,
                                                     dataIndex: 'wtUnit',
                                                     text: 'Weight Unit',
                                                     editor: {
                                                         xtype: 'combobox',
                                                         id: 'wtUnit',
                                                         itemId: 'wtUnit',
                                                         name: 'wtUnit',
                                                         store: [
                                                             'KG',
                                                             'TON'
                                                         ]
                                                     }
                                                }
                                            ],
                                            dockedItems: [
                                                {
                                                	xtype: 'toolbar',
                                                    dock: 'top',
                                                    items: [
                                                        {
                                                            xtype: 'button',
                                                            id: 'addCntr',
                                                            itemId: 'addCntr',
                                                            text: 'Add'
                                                        },
                                                        {
                                                            xtype: 'button',
                                                            id: 'deleteCntr',
                                                            itemId: 'deleteCntr',
                                                            text: 'Delete'
                                                        },
                                                        {
                                                            xtype: 'button',
                                                            id: 'copyCntr',
                                                            itemId: 'copyCntr',
                                                            text: 'Copy'
                                                        }
                                                    ]
                                                    
                                                }
                                            ],plugins: [
                                                {
                                                    ptype: 'cellediting'
                                                }
                                            ],
                                            selModel: {
                                                selType: 'checkboxmodel'
                                            }
                                        },
                                        {
                                            xtype: 'container',
                                            flex: 1,
                                            items: [
                                                {
                                                    xtype: 'checkboxgroup',
                                                    flex: 1,
                                                    width: 400,
                                                    items: [
                                                        {
                                                            xtype: 'checkboxfield',
                                                            name: 'validWt',
                                                            boxLabel: 'Valid Weight',
                                                            bind: '{shipment.validWt}',
                                                            inputValue: '1'
                                                        },
                                                        {
                                                            xtype: 'checkboxfield',
                                                            name: 'approveDoc',
                                                            boxLabel: 'Valid Documents',
                                                            bind: '{shipment.approveDoc}',
                                                            inputValue: '1'
                                                        },
                                                        {
                                                            xtype: 'checkboxfield',
                                                            name: 'goodCustomer',
                                                            boxLabel: 'Good Customer',
                                                            bind: '{shipment.goodCustomer}',
                                                            inputValue: '1'
                                                        }
                                                    ]
                                                }
                                            ]
                                        }
                                    ]
                                }
                            ]
                        }
                    ]
                }
            ]
        }
    ],

    onContainerAfterRender: function(component, eOpts) {
        Ext.define('MyMap', {
            extend: 'Ext.ux.Carousel',
            alias: 'widget.mycarousel',
                        autoShow: true,
                        layout: 'fit',
                        title: 'GMap Window',
                        closeAction: 'hide',
                        width:450,
                        height:450,
                        border: false,
                        x: 40,
                        y: 60,
                        items: {
                            xtype: 'gmappanel',
                            center: {
                                geoCodeAddr: '4 Yawkey Way, Boston, MA, 02215-3409, USA',
                                marker: {title: 'Fenway Park'}
                            },
                            markers: [{
                                lat: 42.339641,
                                lng: -71.094224,
                                title: 'Boston Museum of Fine Arts',
                                listeners: {
                                    click: function(e){
                                        Ext.Msg.alert('It\'s fine', 'and it\'s art.');
                                    }
                                }
                            },{
                                lat: 42.339419,
                                lng: -71.09077,
                                title: 'Northeastern University'
                            }]
                        }
                    });
    },

    onBookingFromCityBlur: function(component, event, eOpts) {
        var from = Ext.getCmp('bookingFromCity').getValue(),
            to = Ext.getCmp('bookingToCity').getValue();

        if (from == to){
            alert('From City should not be the same as To City! Please change');
        }
    },

    onBookingToCityBlur: function(component, event, eOpts) {
        var from = Ext.getCmp('bookingFromCity').getValue(),
            to = Ext.getCmp('bookingToCity').getValue();

        if (from == to){
            alert('From City should not be the same as To City! Please change');
        }
    }

});